...
import data_utils
from architectures import build_simple_qnn, simple_num_params
...
def train(
    ...
    num_params = simple_num_params(n_feature_qubits, n_layers, layer_type)
    ...
    default_run_name = (
        f"simple_qnn_opt-{optimizer_name}_fq{n_feature_qubits}_"
        f"layers{n_layers}_{layer_type}_lr{lr}"
    )
    ...
    print(
        ...
        f"qubits={n_feature_qubits + 1}, params={num_params}, "
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Simple QNN on encoded NF-UNSW data")
    ...
    parser.add_argument("--layer-type", type=str, default="XXYY", choices=["XXYY", "ZZXX", "ZZYY", "ZZXXYY"], help="Entangling layer type")
    ...
    parser.add_argument("--save-dir", type=str, default="outputs/simple", help="Directory where final artifacts are saved")
